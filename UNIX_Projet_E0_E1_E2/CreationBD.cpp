...
ETUDIANT Elm[] = 
{
  {"Wagner","abc","0498.65.56.89","wagner@hepl.be"},
  {"Quettier","123","0476.11.12.13","quettier@gmail.com"},
  {"Wilvers","xyz","0477.22.22.88","wilvers@hepl.be"}
};
...
int main(int argc,char *argv[])
{
  ...
  for (int i=0 ; i<3 ; i++)
  {
	  sprintf(requete,"insert into UNIX_FINAL values (NULL,'%s','%s','%s','%s');",Elm[i].nom,Elm[i].motDePasse,Elm[i].gsm,Elm[i].email);
